...
use crate::blocked_email_service::BlockedEmailError;
use crate::config::Config;
use crate::{
    audit_service, auth_token_service, instance_service, legal_service, turnstile_service,
};
use chrono::Utc;
use domain::{
...
pub enum AuthError {
    ...
    EmailNotVerified { resend_token: String },
    ...
    #[error(transparent)]
    Audit(#[from] audit_service::AuditError),

    #[error(transparent)]
    Legal(#[from] legal_service::LegalError),

    #[error(transparent)]
    Turnstile(#[from] turnstile_service::TurnstileError),
...
pub async fn register(
    ...
    turnstile_service::verify(config, &payload.turnstile_token, client_ip).await?;
    ...
    legal_service::record_both(&txn, user.id, &payload.legal, &ctx).await?;
...
pub async fn login(
    ...
    if email.verified_at.is_none() {
        let resend_token = auth_token_service::issue(
            db,
            user.id,
            email.id,
            domain::types::TokenKind::EmailVerification,
        ...
        return Err(AuthError::EmailNotVerified { resend_token });
    }

    // Article 23: continued use after the notice period counts as
    // acceptance. Recorded here rather than on the request path: at most one
    // write per user per revision, and a sign-in is the moment at which
    // continued use is actually observed.
    if let Err(err) = legal_service::apply_tacit_acceptance(
        db,
        user.id,
        user.legal_terms_version.as_deref(),
        user.legal_privacy_version.as_deref(),
        Utc::now(),
        &ctx,
    )
    .await
    {
        // Evidentiary only. Refusing a sign-in because a proof row could not
        // be written would be worse than the missing row.
        tracing::error!(error = ?err, user_id = %user.id, "failed to record tacit legal acceptance");
    }
...
pub async fn password_forgot(
    ...
    turnstile_service::verify(config, turnstile_token, client_ip).await?;
    ...
    match found {
        Some((user, email_row)) => {
            ...
            if check_account_suspended(&user).is_some() {
                tracing::warn!(user_id = %user.id, "password reset requested for suspended account");
                return Ok(());
            }

            if email_row.verified_at.is_none() {
                tracing::warn!(user_id = %user.id, "password reset requested for unverified email");
                return Ok(());
            }
        ...
        None => {
            // Do not reveal whether the email exists. The caller should always get a success response.
            // Log the attempt for monitoring, but do not return an error to the user.
            tracing::warn!(email = %email, "password reset requested for non-existent email");
        }
    }
...
pub async fn password_reset(
    ...
    // A token issued before the account was suspended stays technically valid.
    // Refuse it here rather than let the reset go through and be blocked at the
    // next sign-in, so no state changes for an account that cannot be used.
    if let Some(suspension) = check_account_suspended(&user) {
        return Err(suspension);
    }
...
pub async fn password_change(
    ...
    .await
    {
        tracing::error!(error = ?err, user_id = %user.id, "failed to send password change alert");
    }
...
pub async fn verify_email(db: &DatabaseConnection, secret: &str) -> Result<(), AuthError> {
    let existing =
        auth_token_service::find(db, secret, domain::types::TokenKind::EmailVerification).await?;
    let email_id = existing.email_id.ok_or(AuthError::InvalidCredentials)?;

    if existing.used_at.is_some() {
        let email = user_emails::Entity::find_by_id(email_id)
            .one(db)
            .await?
            .ok_or(AuthError::InvalidCredentials)?;

        // Already verified: the goal state is reached, so this is a success
        // regardless of the token's own expiry. Refusing here would turn a
        // double-click on the email link into an error for no benefit.
...
/// Re-sends the verification link for a user's address.
...
pub fn suspension_error(
    ...
    if purge_scheduled_at.is_some() {
        return Some(AuthError::PendingDeletion);
    }
...
async fn reject_if_compromised(config: &Config, password: &str) -> Result<(), AuthError> {
    ...
    match password_service::is_compromised(password).await {
        ...
        Ok(false) => Ok(()),
        Err(err) => {
            // Fail open: a network failure or a temporary API outage must not block the user from signing up or changing their password. The risk of a compromised password is real, but the risk of a failed signup is immediate and certain.
